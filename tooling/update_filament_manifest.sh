...
android_hash=$(LC_ALL=C find "$ROOT_DIR/tooling/filament-android-maven" -type f -print0 | LC_ALL=C sort -z | xargs -0 shasum -a 256 | awk '{print $1}' | shasum -a 256 | awk '{print $1}')
ios_hash=$(LC_ALL=C find "$ROOT_DIR/ios/Filament.xcframework" -type f -print0 | LC_ALL=C sort -z | xargs -0 shasum -a 256 | awk '{print $1}' | shasum -a 256 | awk '{print $1}')
